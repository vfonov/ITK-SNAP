...
::InitializeActivationFlags()
{
  ...
  m_Activation->AddMenuItem(m_MenuSaveGrey, UIF_IRIS_WITH_BASEIMG_LOADED);
  ...
  //Added widgets for merging labels and defining boundaries
  m_Activation->AddMenuItem(m_MenuBoundaryLayerOptions, UIF_IRIS_WITH_BASEIMG_LOADED);
  m_Activation->AddMenuItem(m_MenuMergeLabels, UIF_IRIS_WITH_BASEIMG_LOADED);
...
: UserInterface()
{
  ...
  // Create the boundary editor window
  m_BoundaryEditorUI = new BoundaryEditorUILogic();
  m_BoundaryEditorUI->MakeWindow();
  m_BoundaryEditorUI->Register(this);
  
  //Create the merge label window
  m_MergeLabelsUI = new MergeLabelsUILogic();
  m_MergeLabelsUI->MakeWindow();
  m_MergeLabelsUI->Register(this);
...
::SetToolbarMode(ToolbarModeType mode)
{
  ...
  for(unsigned int i=0;i<3;i++)
    ...
    case FILL_MODE:
     	m_TabsToolOptions->value(m_GrpToolOptionFill);
     	break;
...
::OnFillHoles(void)
{
  size_t radius = static_cast<size_t>(m_FillRadius->value());
  size_t iterations = static_cast<size_t>(m_FillIterations->value());
  
  m_Driver->FillHoles(radius, iterations, m_ProgressCommand);
}
...
::OnMenuSaveScreenshotSeries(unsigned int iSlice)
{
  ...
  char *path = fl_dir_chooser(
    "Select the directory to save the screenshots", NULL, 0); //BUGFIX, warning since arg 3 is an integer argument with value 0 and NULL is not guaranteed to be 0 on all systems
...
void
UserInterfaceLogic
::OnEditBoundaryLayers()
{
	m_BoundaryEditorUI->DisplayWindow();
}
...
::OnMergeLabels()
{
	m_MergeLabelsUI->DisplayWindow();
}

void
UserInterfaceLogic
::OnSaveGreyscaleImage()
{
	// Set the history for the wizard
	m_WizGreyIO->SetHistory(m_SystemInterface->GetHistory("GreyImage"));
	m_WizGreyIO->SetSegmentation(m_Driver->GetCurrentImageData()->GetSegmentation()->GetImage());
	// Save the grey image
	list<pair<string, LabelType> > labels;
	for (size_t i = 0; i < MAX_COLOR_LABELS; ++i)
	{
		ColorLabel label = m_Driver->GetColorLabelTable()->GetColorLabel(i);
		if (label.IsValid())
			labels.push_back( pair<string, LabelType>(string(label.GetLabel()), i));
	}
	
	m_WizGreyIO->SetLabels(labels);
	
	if(m_WizGreyIO->DisplaySaveWizard(m_Driver->GetCurrentImageData()->GetGrey()->GetImage(),NULL))
	{
		// Update the history for the wizard
	  	m_SystemInterface->UpdateHistory("GreyImage",m_WizGreyIO->GetSaveFileName());
	}
}

/*
 *$Log: UserInterfaceLogic.cxx,v $
 *Revision 1.25.2.1  2008/07/03 20:31:10  timburke1661
